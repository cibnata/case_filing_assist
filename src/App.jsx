...
const OCR_API_BASE = import.meta.env.VITE_OCR_API_BASE || "";
const LLM_API_BASE = import.meta.env.VITE_LLM_API_BASE || "";

const ocrBases = [OCR_API_BASE, "/ocr-api", "http://localhost:8000"].filter(Boolean);
const llmBases = [LLM_API_BASE, "/llm-api", "http://localhost:8001"].filter(Boolean);

const fetchWithBaseFallback = async (bases, path, options) => {
  let lastError = null;
...
function EvidenceTab({ caseData, setCaseData, setActiveTab }) {
  ...
  const runOCR = async (ev) => {
    ...
    try {
      ...
      console.log(`發送請求至 Surya Server 候選端點: ${ocrBases.join(" | ")}`);
      const { res: ocrResponse, endpoint: ocrEndpoint } = await fetchWithBaseFallback(ocrBases, "/ocr", {
        method: "POST",
        body: formData,
        signal: controller.signal,
      });
      console.log(`OCR 使用端點: ${ocrEndpoint}`);
...
- JSON 物件必須完整且格式正確。`;
	  ...
	  console.log(`發送請求至 LLM Server 候選端點: ${llmBases.join(" | ")}`);
      const { res: llmResponse, endpoint: llmEndpoint } = await fetchWithBaseFallback(llmBases, "/v1/chat/completions", {
        method: "POST",
      ...
      console.log(`LLM 使用端點: ${llmEndpoint}`);
      ...
      const hint = err?.message === "Failed to fetch"
        ? `連線失敗，請確認 OCR/LLM 服務是否啟動；已嘗試 OCR: ${ocrBases.join(" | ")}；LLM: ${llmBases.join(" | ")}（並確認 https/http 無混合內容）`
        : err.message;
      addLog("ocr_fail", `${ev.number} 失敗: ${hint}`);
    } finally {
      setProcessing(null);
    }
  };
...
function ExtractTab({ caseData, setCaseData }) {
  ...
  useEffect(() => {
    ...
    const graphBase = ocrBases[0] || "/ocr-api";
    fetch(`${graphBase}/network-graph`, {
      method: "POST",
      headers: { "Content-Type": "application/json" },
